...
def _generate_manifest(root: Path) -> tuple[Dict[str, Any], str]:
...
def run_self_test(backpack_path: Path, quiet: bool = False) -> bool:
    ...
    try:
        ...
        # Try finding it in tests/ relative to the repo root (src-layout)
        _src_dir = Path(__file__).resolve().parent  # src/provara/
        for candidate in [
            _src_dir.parent.parent / "tests",   # repo/tests/
            _src_dir.parent / "test",           # legacy SNP_Core/test/
        ]:
