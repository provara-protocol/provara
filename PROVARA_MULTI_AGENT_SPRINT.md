# PROVARA MULTI-AGENT SPRINT — Orchestration Prompt

> **Use this document as a dispatcher.** Paste the relevant AGENT BRIEF (plus shared context) into each agent's session. The owner coordinates handoffs. Agents never talk to each other — the repo is the shared state.

---

## HOW TO USE THIS

1. **Copy the SHARED CONTEXT block** into every agent session (it's the "constitution")
2. **Copy one AGENT BRIEF** per agent (each agent gets its own lane)
3. **Kick off all agents in parallel** — they're designed to be non-blocking
4. **Collect outputs** and commit them yourself, or let agents with repo access commit
5. **Run the SYNC CHECK** between sprints to catch conflicts
...
- 225+ tests (unit + vector + compliance + PSMC + MCP)
...
4. ALWAYS run all tests before claiming done
...
## AGENT BRIEF: ARCHITECT (Claude Opus / Claude Code)
...
## AGENT BRIEF: RUSTACEAN (Claude Code / Codex)
...
## AGENT BRIEF: HARDENER (Gemini / Claude Code)
...
## AGENT BRIEF: INTEGRATOR (Codex / Claude Code)
...
## SYNC CHECK — Run Between Sprints

After collecting outputs from all agents, the owner (or orchestrating agent) runs this checklist:

```
□ All tests pass: `make test` (225+ tests, 0 failures)
□ No PROTOCOL_PROFILE.txt modifications
□ No new dependencies added without approval
□ No PII or OPSEC violations in any output
□ TODO.md updated with completed items and new discoveries
□ No conflicting file modifications between agents
□ All new files follow existing naming conventions
□ Commit messages follow Conventional Commits format
□ All docs reference PROTOCOL_PROFILE.txt for crypto claims
□ Any spec ambiguities discovered are logged in docs/OPEN_DECISIONS.md
```
...
```
Day 0:  Owner dispatches all 5 agent briefs in parallel
Day 1:  Agents work independently, commit to feature branches
Day 2:  Owner runs SYNC CHECK, resolves conflicts, merges to main
Day 3:  Owner reviews, provides feedback, updates TODO.md
Day 4:  Agents iterate on feedback + pick up next priorities from TODO.md
Day 5:  Final SYNC CHECK, merge, tag release if warranted
```
...
| Situation | Action |
|-----------|--------|
| Spec ambiguity discovered | Document in `docs/OPEN_DECISIONS.md`, make a decision, flag for owner |
| Another agent's code looks wrong but tests pass | Note in TODO.md, do not refactor |
| Another agent's code fails tests | Fix it if trivial, flag owner if not |
| New dependency needed | STOP. Flag owner. Do not install. |
| OPSEC violation found | Do not propagate. Flag owner immediately with file path + line number |
| Blocked on owner action (billing, accounts, keys) | Document the blocker in TODO.md, move to next task |

---

## THE META-PROMPT — For Kicking Off Each Agent

When pasting into a new agent session, use this wrapper:

```
You are working on the Provara Protocol as part of a coordinated multi-agent sprint.

[PASTE SHARED CONTEXT HERE]

[PASTE YOUR SPECIFIC AGENT BRIEF HERE]

Your working branch: `sprint/[agent-name]-[date]`

Start by:
1. Reading AGENTS.md and PROTOCOL_PROFILE.txt from the repo
2. Checking TODO.md for current state
3. Confirming your understanding of your deliverables
4. Beginning work on your Primary task

Output format for every deliverable:
- Files created/modified (with paths)
- Test evidence (commands run + output)
- Handoff notes for owner
- TODO.md updates (items completed, items discovered)
